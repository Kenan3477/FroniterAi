/**
 * Restore Omnivox Call Recordings from Twilio - FIXED VERSION
...
async function restoreOmnivoxRecordingsFixed() {
    try {
        ...
        console.log('🔄 Restoring Omnivox recordings from Twilio (FIXED VERSION)...\n');

        // First, ensure we have the required dependencies
        await ensureDependenciesFixed();
        ...
        if (restoredRecordings > 0) {
            console.log('\n✅ SUCCESS! Your Omnivox recordings have been restored! Check the frontend now.');
        } else {
...
async function ensureDependenciesFixed() {
    try {
        // Ensure campaign exists with correct schema
        await prisma.campaign.upsert({
            ...
            create: {
                ...
                status: 'ACTIVE',
                dialMethod: 'MANUAL',  // Use dialMethod instead of dialMode
                speed: 1.0,
                recordCalls: true,
                createdBy: 'SYSTEM'
            }
        ...
        console.log('✅ Dependencies created/verified');
